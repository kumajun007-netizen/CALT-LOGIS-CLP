...
# 💡 PKG NO 정렬을 위한 자연어 인식기 (예: PKG-2가 PKG-10보다 앞에 오도록 처리)
def nat_key(s):
...
def calculate_expert_packing(df, max_40_wt, max_40_len, max_20_wt, max_20_len, max_dry_h, max_hc_h, load_mode):
    ...
    # 💡 정렬 기준(sk)에 PKG NO 추가! 
    # 크기, 화주(GROUP)가 같으면 PKG NO 숫자가 작은 순서대로 적재를 진행합니다.
    sk=lambda x:(-x['W'],-x['H'],-x['L'],x['GROUP'], nat_key(x.get('PKG NO', '')))
